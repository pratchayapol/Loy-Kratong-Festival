...
<body class="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 font-display">
    <div class="grid min-h-screen lg:grid-cols-[420px_1fr] gap-4">
        ...
        <aside class="p-4 lg:p-6">
            <div class="backdrop-blur-xl rounded-2xl border border-white/10 bg-glass shadow-glass"
                x-data="krathongForm()">
                <div class="p-5 lg:p-6 border-b border-white/10">
                    <h1 class="text-2xl font-semibold tracking-tight">ลอยกระทง</h1>
                    <p class="text-sm text-slate-300 mt-1">เลือกแบบกระทง กรอกชื่อ อายุ และคำอธิษฐาน แล้วปล่อยลอยได้ทันที
                    </p>
                </div>
                    ...
                    <div>
                       ...
                       ring-0 data-[checked=true]:ring-2 data-[checked=true]:ring-cyan-400/60 p-3 flex flex-col items-center gap-2"
                                    ...
                                    <img src="{{ $t['img'] }}" alt="{{ $t['label'] }}"
                                        class="w-12 h-12 drop-shadow" loading="lazy">
                                    <span class="text-sm">{{ $t['label'] }}</span>
                                    <span
                                        class="absolute -top-2 -right-2 hidden data-[checked=true]:inline-flex items-center justify-center
        ...
        <main class="relative">
            ...
            <div class="absolute inset-0 overflow-hidden" x-data="riverScene(@js($types), @js($recent))" @dblclick="spawnRandom()">
                <div
                    class="absolute left-0 w-[200%] h-28 top-[12%] opacity-30 blur-2xl bg-[radial-gradient(ellipse_at_center,_white_0%,_transparent_60%)] animate-waves">
                </div>
                <div
                    class="absolute left-0 w-[200%] h-28 top-[44%] opacity-25 blur-2xl bg-[radial-gradient(ellipse_at_center,_white_0%,_transparent_60%)] animate-[waves_22s_linear_infinite]">
                </div>
                <div
                    class="absolute left-0 w-[200%] h-28 top-[72%] opacity-20 blur-2xl bg-[radial-gradient(ellipse_at_center,_white_0%,_transparent_60%)] animate-[waves_26s_linear_infinite]">
                </div>

                <!-- Floating krathongs -->
                <template x-for="k in items" :key="k.clientId">
                    <div class="absolute flex flex-col items-center animate-floatY will-change-transform"
                        :style="k.style">
                        <div class="px-3 py-1.5 rounded-2xl text-sm max-w-[260px] text-cyan-50/95 bg-slate-900/60 backdrop-blur
                        border border-white/10 shadow ring-1 ring-white/10"
                            x-text="k.wish"></div>
                        <img :src="k.img" alt="krathong"
                            class="w-20 h-auto drop-shadow-[0_12px_18px_rgba(0,0,0,0.45)] mt-1.5">
                    </div>
                </template>

                <div
                    class="pointer-events-none absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-slate-950/70 to-transparent">
                </div>
            </div>
        </main>
    </div>
    ...
    <script>
        function riverScene(types, recent) {
            ...
            const makeStyle = (dur, delay, topPct) => {
                ...
                sheet.insertRule(`@keyframes ${name}{0%{transform:translateX(-12%)}100%{transform:translateX(112%)}}`,
