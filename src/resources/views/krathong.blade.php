...
<body x-data="{}" class="min-h-screen bg-slate-950 text-slate-100 font-display overflow-hidden">
  ...
  <main class="relative min-h-screen">
    ...
    <div class="absolute top-0 left-0 right-0 h-[58%]">
      ...
      <!-- ดาวระยิบ -->
      <div class="pointer-events-none absolute inset-0" x-data="{ stars: [] }" x-init="const count=Math.round(Math.min(180,90+window.innerWidth/8));for(let i=0;i<count;i++){const s=Math.random();stars.push({left:Math.random()*100,top:Math.random()*100,delay:Math.random()*5,duration:3+Math.random()*6,size:s>0.92?2.5:(s>0.75?1.5:1),opacity:s>0.85?1:0.7});}">
    ...
    <!-- HORIZON โค้ง -->
    <svg class="absolute left-0 right-0 top-[58%] h-20 pointer-events-none" viewBox="0 0 1440 160" preserveAspectRatio="none" aria-hidden="true">
      <!-- เส้นไฮไลต์ -->
        ...
        <template x-for="k in items" :key="k.clientId">
          <div class="absolute flex flex-col items-center will-change-transform krathong-item" :style="k.style">
            ...
            <div class="relative mt-2">
              <img :src="k.img" alt="krathong" class="w-16 h-16 sm:w-20 sm:h-20 drop-shadow-[0_15px_25px_rgba(0,0,0,0.6)] relative z-10">
  ...
  <div x-show="$store.ui.open" x-cloak class="fixed inset-0 z-50" @keydown.escape.window="$store.ui.open=false">
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4 sm:p-6 overflow-y-auto" @click.stop>
      <div class="w-full max-w-xl modal-enter backdrop-blur-2xl rounded-3xl border border-white/20 bg-slate-900/50 shadow-glass my-8" x-data="krathongForm()">
        <div class="flex items-start justify-between p-5 sm:p-6 border-b border-white/10">
          <div>
            <h2 class="text-2xl font-bold bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 bg-clip-text text-transparent">ลอยกระทง</h2>
        ...
        <form @submit.prevent="submit" class="p-5 sm:p-6 space-y-5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="space-y-2">
              ...
              <input x-model="form.nickname" type="text" maxlength="50" required class="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:border-cyan-400/60 focus:ring-2 focus:ring-cyan-400/30 transition-all placeholder:text-slate-500" placeholder="เช่น โฟกัส">
            </div>
            <div class="space-y-2">
              <label class="text-sm font-semibold text-slate-200">อายุ</label>
              <input x-model.number="form.age" type="number" min="1" max="120" required class="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:border-cyan-400/60 focus:ring-2 focus:ring-cyan-400/30 transition-all">
            </div>
          </div>

          <div class="space-y-2">
            <label class="text-sm font-semibold text-slate-200">คำอธิษฐาน</label>
            <textarea x-model="form.wish" maxlength="200" required rows="3" class="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 outline-none focus:border-cyan-400/60 focus:ring-2 focus:ring-cyan-400/30 transition-all placeholder:text-slate-500 resize-none" placeholder="ขอให้..."></textarea>
          ...
          <div class="flex flex-wrap items-center gap-3 pt-2">
            <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 px-6 py-3 font-semibold shadow-btn hover:shadow-[0_15px_50px_rgba(34,211,238,0.55)] hover:scale-105 active:scale-100 transition-all duration-300">
              <svg xmlns="http://www.w3.org/2000/svg" class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.5 12a7.5 7.5 0 0015 0m-15 0a7.5 7.5 0 0115 0M12 3v9"/></svg>
              ลอยเลย
            </button>
            <button type="button" @click="$store.ui.open=false" class="rounded-xl border border-white/20 px-5 py-3 hover:bg-white/10 transition-colors font-medium">ปิด</button>
  ...
  <script>
    ...
    function riverScene(types, recent) {
      const WATER_TOP = 35; // เริ่มน้ำที่ 58% ของจอ
      const WATER_BAND = 42;
      ...
      return {
        ...
        init() {
          ...
          const tick = () => { this._spawnNext(false); __schedule(tick, rnd(4500, 7200)); };
          __schedule(tick, 900);
        },
        ...
        _spawnNext(isInitial) {
          ...
          let guard = 0;
          while (this.seenInCycle.has(this.order[this.idx]?.id) && guard < this.order.length) { this.idx = (this.idx + 1) % this.order.length; guard++; }
        ...
        spawnNew(p) {
          const r = { id: p.id ?? Date.now(), type: p.type, nickname: p.nickname, age: p.age, wish: p.wish };
